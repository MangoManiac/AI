...
param_p1 = np.load(p1_model, allow_pickle=True)
...
param_p2 = np.load(p2_model, allow_pickle=True)
...
X2 = X[ids == 2]
 ...
 # modified the scatter code
plt.scatter(X1[:, 0] - min_f1, X1[:, 1] - min_f2, marker='.', color='red', label='Phoneme 1')
plt.scatter(X2[:, 0] - min_f1, X1[:, 1] - min_f2, marker='.', color='green', label='Phoneme 2')
